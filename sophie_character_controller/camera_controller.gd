...
## Distance from character to camera
...
func _physics_process(delta: float) -> void:
	...
	# Update camera position to maintain character in center
	...
	var camera_offset := -basis.z * camera_distance
	_camera.global_position = target_position + camera_offset
	_camera.look_at(_anchor.global_position + Vector3.UP * 1.5)  # Aim slightly above character center
...
func setup(anchor: CharacterBody3D) -> void:
	_anchor = anchor
	_offset = Vector3.ZERO  # Reset offset since we're maintaining center position
